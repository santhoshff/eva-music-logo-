...
export const ThemeToggleButton = ({
  ...
  return (
    <button
      type="button"
      className={cn(
        "size-10 cursor-pointer rounded-full bg-black p-0 transition-all duration-300 active:scale-95 shadow-sm border border-black/10 dark:border-white/20 dark:bg-slate-900",
        className,
      )}
      onClick={toggleTheme}
    ...
    >
      ...
      <svg viewBox="0 0 240 240" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full h-full p-2">
        <motion.g
...
export const Options = ({
  ...
  return (
    <motion.div
      drag
      className="top-24 border-foreground/10 bg-white/90 dark:bg-slate-900/90 text-slate-800 dark:text-slate-100 shadow-2xl absolute right-1/2 flex w-[260px] translate-x-1/2 flex-col gap-3 rounded-3xl border p-4 backdrop-blur-xl z-50 lg:right-4 lg:translate-x-0"
    >
      <div className="flex items-center justify-between">
        ...
        <p className="group flex cursor-pointer items-center justify-center gap-1 rounded-lg px-2 py-0.5 text-xs font-semibold opacity-70">
          Animation Settings
        </p>
        ...
        {onClose && (
          <button
            onClick={onClose}
            className="text-xs px-1.5 py-0.5 rounded-full hover:bg-slate-200 dark:hover:bg-slate-800 opacity-60 hover:opacity-100"
          >
            ✕
          </button>
        )}
      ...
      <div className="flex flex-col text-xs">
        <div className="mt-1 flex justify-between py-1 items-center">
          <p className="w-16 whitespace-nowrap opacity-60 font-medium">variant :</p>
          <div className="flex flex-wrap items-center justify-end gap-1">
            {(["circle", "rectangle", "gif", "polygon", "circle-blur"] as AnimationVariant[]).map((v) => (
              <button
                ...
                className={cn(
                  "cursor-pointer px-1.5 py-0.5 rounded text-[11px] transition-all",
                  variant === v
                    ? "bg-purple-600 text-white font-semibold shadow-xs"
                    : "hover:bg-purple-500/10 opacity-60 hover:opacity-100",
                )}
              >
                {v}
              </button>
        ...
        <div className="mt-1 flex justify-between py-1 items-center">
          <p className="w-16 whitespace-nowrap opacity-60 font-medium">blur :</p>
          <div className="flex flex-wrap items-center justify-end gap-1">
            <button
              onClick={() => setBlur(false)}
              className={cn(
                "cursor-pointer px-2 py-0.5 rounded text-[11px] transition-all",
                !blur
                  ? "bg-purple-600 text-white font-semibold shadow-xs"
                  : "hover:bg-purple-500/10 opacity-60 hover:opacity-100",
              )}
        ...
        {(variant === "circle" ||
          ...
          variant === "circle-blur") && (
          <div className="mt-1 flex justify-between py-1 items-center">
            <p className="w-16 whitespace-nowrap opacity-60 font-medium">start :</p>
            <div className="flex flex-wrap items-center justify-end gap-1">
              ...
              {(variant === "circle" ||
                ...
                variant === "circle-blur") && (
                <>
                  {(["top-left", "top-right"] as AnimationStart[]).map((s) => (
                    <button
                      key={s}
                      onClick={() => setStart(s)}
                      className={cn(
                  ...
                  {variant !== "polygon" && (
                    <>
                      {(["bottom-left", "bottom-right"] as AnimationStart[]).map((s) => (
                        <button
                          key={s}
                          onClick={() => setStart(s)}
                          className={cn(
              ...
              {(variant === "circle" || variant === "circle-blur") && (
                <>
                  {(["top-center", "bottom-center"] as AnimationStart[]).map((s) => (
                    <button
                      key={s}
                      onClick={() => setStart(s)}
                      className={cn(
        ...
        {variant === "gif" && (
          <div className="mt-1 flex justify-between py-1 items-center">
            <p className="w-16 opacity-60 font-medium">gif type :</p>
            <div className="flex flex-wrap items-center justify-end gap-1">
              <button
                onClick={() => {
                  setGifType("1");
        ...
        {variant === "gif" && gifType === "custom" && (
          <div className="mt-1 flex flex-col gap-1 py-1">
            <p className="opacity-60 font-medium">gif url :</p>
            <input
              type="text"
              value={gifUrl}
              onChange={(e) => setGifUrl(e.target.value)}
              placeholder="Enter GIF URL"
              className="text-foreground border border-slate-300 dark:border-slate-700 placeholder:opacity-50 w-full rounded-lg bg-transparent px-2 py-1 text-xs focus:outline-none focus:border-purple-500"
            />
          </div>
        )}
      </div>
